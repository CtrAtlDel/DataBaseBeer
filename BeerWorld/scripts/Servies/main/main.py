...
try:
    ...
    #     cur.execute('INSERT INTO "Sort"(sort_name) VALUES (%s)',[str(sorts[i])])
        
    for i in range(0, 9990):    
        sort = str(get_random_str(10))
        arr_sort.add(sort)
        cur.execute('INSERT INTO "Sort"(sort_name) VALUES (%s)',[str(sort)])
    ...
    for i in range(0, 10000):
        institutionName = get_random_str(10)
        arr_institution_name.add(institutionName)
        cur.execute('INSERT INTO "Institution" (name, address, phone_number, type, status) \
            VALUES (%s, %s, %s, %s, %s)',
            (random.choice(institutionName),
            get_random_str(15),
            random_phone_num_generator(),
            random.choice(typeInstitution),
            random.choice(statusInstitution))
        )
    #Dependency
    conn.commit()
